...
function createWindow() {
    // `app.getAppPath()` points to the packaged app root (e.g. `.../resources/app.asar`)
    // and to the project root in dev. Using it avoids fragile relative paths inside asar.
    ...
    mainWindow = new BrowserWindow({
        ...
        icon: path.join(appRoot, "public", "vite.svg"),
    });
    ...
    if (process.env.NEXT_DEV_SERVER_URL) {
        ...
        mainWindow.loadFile(path.join(appRoot, "dist-next", "index.html"));
    }
...
// System tray
function createTray() {
    const icon = nativeImage.createFromPath(path.join(app.getAppPath(), "public", "vite.svg"));
    tray = new Tray(icon);
